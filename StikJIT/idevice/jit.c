...
// New function to handle USB debugging
int debug_app_usb(UsbmuxdAddrHandle* addr, const char *bundle_id, LogFuncC logger) {
    ...
    // Create USB provider
    logger("Creating USB provider for debugging...");
    UsbmuxdProviderHandle *usb_provider = NULL;
    err = usbmuxd_provider_new(addr, 1, "", 0, "StikJIT", &usb_provider);
    if (err != IdeviceSuccess) {
        logger("Failed to create USB provider: %d", err);
        return 1;
    }
    
    // Connect to CoreDeviceProxy over USB
    CoreDeviceProxyHandle *core_device = NULL;
    err = core_device_proxy_connect_usbmuxd(usb_provider, &core_device);
    if (err != IdeviceSuccess) {
        logger("Failed to connect to CoreDeviceProxy over USB: %d", err);
    ...
    logger("Debug session completed over USB");
    return 0;
}
